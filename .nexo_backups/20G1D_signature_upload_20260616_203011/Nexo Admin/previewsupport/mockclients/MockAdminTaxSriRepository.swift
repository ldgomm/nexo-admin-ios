...
actor MockAdminTaxSriRepository: AdminTaxSriRepository {
    ...
    func uploadSignature(_ input: UploadAdminElectronicSignatureInput) async throws -> AdminElectronicSignature {
        let signature = AdminElectronicSignature(
            id: "sig_\(signatures.count + 1)",
            organizationId: "org_altos",
            alias: input.alias,
            subject: "Certificado cargado",
    ...
    func activateSignature(_ input: AdminSignatureActionInput) async throws -> AdminElectronicSignature {
        signatures = signatures.map {
            AdminElectronicSignature(
                ...
                validTo: $0.validTo,
                status: $0.status,
                effectiveStatus: $0.effectiveStatus,
                usable: false,
                ...
                lastValidatedAt: $0.lastValidatedAt,
    ...
    private func updateSignature(_ id: String, status: String, active: Bool?) throws -> AdminElectronicSignature {
        ...
        let usable = active ?? old.usable

        let next = AdminElectronicSignature(
            id: old.id,
            ...
            usable: usable,
            expiresInDays: old.expiresInDays,
            expiresSoon: old.expiresSoon,
