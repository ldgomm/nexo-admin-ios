...
enum AdminElectronicDocumentMapper {
    ...
    private static func mapTimeline(_ dto: AdminElectronicDocumentTimelineEventDTO) -> AdminElectronicDocumentTimelineEvent {
        AdminElectronicDocumentTimelineEvent(
            id: dto.id ?? UUID().uuidString,
            type: dto.type ?? "event",
            title: dto.title ?? AdminElectronicDocumentText.statusTitle(dto.type ?? "Evento"),
            message: dto.message ?? "Evento documental registrado.",
            actor: dto.actor,
            createdAt: dto.createdAt ?? "—",
            severity: AdminSriErrorSeverity(rawValue: dto.severity?.lowercased() ?? "") ?? .info
        )
    }
}
